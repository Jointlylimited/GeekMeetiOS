...
class MatchLikesViewController: UIViewController {
    ...
    func setUI()
    {
        ...
        pageMenu = CAPSPageMenu(viewControllers: controllerArray, frame: CGRect(x: 0.0, y: DeviceType.hasNotch ? (viewNavigation?.frame.height)! + 44 : ((viewNavigation?.frame.height)! + 24), width: self.view.frame.width, height: DeviceType.hasNotch ? ((self.view.frame.height - 44)  - 100) : ((self.view.frame.height - 44)  - 80)), pageMenuOptions: parameters)
